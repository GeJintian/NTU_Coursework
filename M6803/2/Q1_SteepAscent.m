...
%[0 19 -1 7 -3; 10 0 0 0 0; -1 0 0 0 0; 10 0 0 0 0; -2 0 0 0 0] (- 2*x^4 + 10*x^3 - x^2 + 10*x - 3*y^4 + 7*y^3 - y^2 + 19*y)
...
tmp_x = 0;
tmp_y = 0;
[x_min,y_min] = steepest_ascent(0,0,maxit,0.00001,f);
x = x_min;
y = y_min;
...
function [x_prev,y_prev] = steepest_ascent(init_x, init_y, maxit, es, f)
...
%(f(x+a)-f(x))/a, but I suppose directly using diff() should be enough.
syms x y;
dif_x = diff(f,x);
dif_y = diff(f,y);
%plot surface here.
